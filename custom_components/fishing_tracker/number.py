...
class FishLengthNumber(NumberEntity):
    ...
    def __init__(self, entry: ConfigEntry, store) -> None:
        self.entry = entry
        self.store = store
        self._attr_unique_id = f"{entry.entry_id}_length_cm"

    @property
    def native_value(self):
        try:
            return float(self.store.settings.get("length_cm", 0))
        except Exception:
            return 0

    async def async_set_native_value(self, value: float) -> None:
        await self.store.async_set_setting("length_cm", int(value))
