...
def _safe_float(value: Any, default: float = 0.0) -> float:
    try:
        if value in (None, "", "None", "unknown", "unavailable"):
            return default
        return float(value)
    except Exception:
        return default
...
def success(entry: dict[str, Any]) -> bool:
    return _safe_int(entry.get("caught", entry.get("anzahl", 0))) >= 1
...
def weighted_rate(fang: int, total: int, prior: float = 50.0, strength: int = 8) -> float:
    """Bayesian-ish smoothing: avoids fake 100% from tiny samples."""
    if total <= 0:
        return prior
...
def best_by(entries: list[dict[str, Any]], key: str, minimum: int = 2) -> dict[str, Any]:
    ...
    for name, stats in groups.items():
        if stats["gesamt"] < minimum:
            continue
        raw = round(stats["fang"] / stats["gesamt"] * 100, 1)
        smoothed = weighted_rate(stats["fang"], stats["gesamt"])
        # Ranking rewards high rates, but requires sample size.
        rank_score = smoothed * min(1.0, stats["gesamt"] / 12)
        if rank_score > best["rate"] * min(1.0, max(best["count"], 1) / 12):
            best = {
                "name": name,
                "rate": smoothed,
                "raw_rate": raw,
...
def combo_key(entry: dict[str, Any]) -> str:
    return f"{entry.get('spot') or 'Unbekannt'} + {entry.get('bait') or entry.get('koeder') or 'Unbekannt'}"
...
def stats(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for e in entries:
        n = dict(e)
        if "bait" not in n and "koeder" in n:
            n["bait"] = n["koeder"]
        if "fish_type" not in n and "fischart" in n:
            n["fish_type"] = n["fischart"]
        if "hour" not in n:
            n["hour"] = parse_hour(n.get("timestamp") or n.get("datum"))
        if "combo" not in n:
            n["combo"] = combo_key(n)
        normalized.append(n)
    ...
    lengths = [normalize_length(e.get("length_cm") or e.get("laenge")) for e in normalized if success(e)]
