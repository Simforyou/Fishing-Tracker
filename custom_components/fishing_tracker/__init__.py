...
SERVICE_ANALYZE_SCHEMA = vol.Schema({
    vol.Required("image_data"): cv.string,   # base64 encoded image
    vol.Required("api_key"):    cv.string,   # Anthropic API key
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_analyze_deeper(call: ServiceCall) -> None:
        ...
        # Ergebnis als persistente Benachrichtigung (Panel liest sie dann ab)
        await hass.services.async_call(
            "persistent_notification",
            "create",
            {
                "message":         _json.dumps(result),
                "title":           "fishing_tracker_deeper_result",
                "notification_id": "fishing_tracker_deeper_result",
            },
            blocking=False,
        )
