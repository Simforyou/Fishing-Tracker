...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_analyze_deeper(call: ServiceCall) -> None:
        ...
        prompt = (
            ...
            "Extrahiere ALLE sichtbaren Tiefenzahlen und ihre Position im Bild. "
            "Beschreibe auch sichtbare Landmarken (Brücken, Straßen, markante Kurven). "
            "Antworte NUR mit validem JSON (kein Markdown):\n"
            '{"depths":[{"value":1.6,"x_pct":45,"y_pct":30,"label":"1.6"}],'
            '"max_depth":2.0,"min_depth":0.6,'
            '"waterway_shape":"Beschreibung",'
            '"landmarks":[{"type":"Brücke","x_pct":30,"y_pct":5,"description":"Details"}],'
            '"depth_unit":"m","coverage_estimate":"ca. 200m"}'
        )
        ...
        if result["success"] and call.data.get("ref_blue_lat", 0) != 0:
            ...
            try:
                ...
                result["scan_name"] = call.data.get("scan_name", "Scan")
                result["timestamp"] = datetime.now().isoformat()
                # Scan dauerhaft akkumulieren
                domain_data = hass.data.get(DOMAIN, {}).get(entry.entry_id, {})
                scans = domain_data.get("deeper_scans", [])
                scans = [s for s in scans if s.get("scan_name") != result["scan_name"]]
                scans.append({"scan_name": result["scan_name"], "timestamp": result["timestamp"],
                              "depth_count": len(depth_pts), "min_depth": min_d, "max_depth": max_d,
                              "depth_points": depth_pts})
                domain_data["deeper_scans"] = scans
