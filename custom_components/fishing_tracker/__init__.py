    ...
    async def handle_analyze_deeper(call: ServiceCall) -> None:
        ...
        if result["success"] and call.data.get("ref_blue_lat", 0) != 0:
            ...
            try:
                ...
                existing = entry_data.get("deeper_scans", [])
                existing = [s for s in existing if s.get("scan_name") != scan_name]
                new_scan = {
                    "scan_name": scan_name,
                    "timestamp": ts,
                ...
                existing.append(new_scan)
                entry_data["deeper_scans"] = existing
                # Scan-Liste auch direkt im result → Panel kann sofort anzeigen
