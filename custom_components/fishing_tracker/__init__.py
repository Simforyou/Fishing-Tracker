...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if not hass.services.has_service(DOMAIN, "update_catch"):
        hass.services.async_register(DOMAIN, "update_catch", handle_update_catch,
            schema=vol.Schema({
                vol.Required("timestamp"): cv.string,
                vol.Optional("fish_type"): vol.Any(cv.string, None),
                vol.Optional("length_cm"): vol.Any(vol.Coerce(float), None),
                vol.Optional("weight_kg"): vol.Any(vol.Coerce(float), None),
                vol.Optional("bait"): vol.Any(cv.string, None),
                vol.Optional("spot"): vol.Any(cv.string, None),
                vol.Optional("notes"): vol.Any(cv.string, None),
                vol.Optional("water_temp"): vol.Any(vol.Coerce(float), None),
                vol.Optional("temperature"): vol.Any(vol.Coerce(float), None),
                vol.Optional("wind_speed"): vol.Any(vol.Coerce(float), None),
                vol.Optional("wind_gusts"): vol.Any(vol.Coerce(float), None),
                vol.Optional("wind_bearing"): vol.Any(vol.Coerce(float), None),
                vol.Optional("pressure"): vol.Any(vol.Coerce(float), None),
                vol.Optional("cloud_coverage"): vol.Any(vol.Coerce(float), None),
                vol.Optional("precipitation"): vol.Any(vol.Coerce(float), None),
