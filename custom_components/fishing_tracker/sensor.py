...
class MapDataSensor(FishingBaseSensor):
    ...
    async def async_update(self) -> None:
        ...
        # Deeper-Analyse Ergebnis aus hass.data lesen (wenn vorhanden)
        from homeassistant.const import DOMAIN as _HA_DOMAIN
        deeper = None
        try:
            domain_data = self.hass.data.get("fishing_tracker", {})
            for entry_data in domain_data.values():
                if isinstance(entry_data, dict) and "deeper_result" in entry_data:
                    deeper = entry_data["deeper_result"]
                    break
        except Exception:
            pass

        # Deeper scans aus hass.data
        deeper_scans = []
        try:
            domain_data = self.hass.data.get("fishing_tracker", {})
            for entry_data in domain_data.values():
                if isinstance(entry_data, dict):
                    deeper_scans = entry_data.get("deeper_scans", [])
                    break
        except Exception:
            pass
