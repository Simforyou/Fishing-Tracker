...
class FishingStore:
    ...
    async def async_load(self) -> None:
        ...
        self.data.setdefault("deeper_scans", [])
        for key, value in DEFAULT_SETTINGS.items():
            self.data["settings"].setdefault(key, value)
        await self.async_save()

    @property
    def deeper_scans(self) -> list:
        return self.data.setdefault("deeper_scans", [])

    async def async_save_deeper_scan(self, scan: dict) -> None:
        scans = self.deeper_scans
        scans = [s for s in scans if s.get("scan_name") != scan.get("scan_name")]
        scans.append(scan)
        self.data["deeper_scans"] = scans
        await self.async_save()

    async def async_delete_deeper_scan(self, scan_name: str) -> bool:
        before = len(self.deeper_scans)
        self.data["deeper_scans"] = [s for s in self.deeper_scans if s.get("scan_name") != scan_name]
        if len(self.deeper_scans) < before:
            await self.async_save()
            return True
        return False
